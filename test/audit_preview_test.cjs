...
setTimeout(()=>{try{
 ...
 for(const type of e('Object.keys(catalog)')){e(`state.active=null;session=null;startQuiz({ids:[questions.find(q=>q.type==='${type}'&&!q.reviewOnly).id],count:1});pauseQuiz();restore()`);assert(e('state.active'),type+' draft retained');e('resume()');assert(e('session'),type+' draft resumed');e('pauseQuiz()');}
 e("state.active=null;session=null;state.memory={};startQuiz({ids:[questions.find(q=>q.type==='choice'&&!q.reviewOnly).id],count:1});session.selected=qnow().options.find(v=>v!==qnow().answer);check()");
 ...
 e("selection={source:'file2',variant:'original'};location.hash='#exam';render()");assert.equal($('#summaryCount').textContent,'38');
 ...
 e("location.hash='#progress';render()");assert.equal(w.document.querySelectorAll('.source-progress').length,6);assert($('#app').textContent.includes('الملف 3'));
